...
const double REWARD_PLACEMENT_SUCCESS = 1.0;
const double PENALTY_INVALID_PLACEMENT = -5.0;
const double PENALTY_DISCONNECTED_HOUSE = -10.0;
const double REWARD_CONSECUTIVE_SUCCESS_BONUS = 3.0;
...
const double REWARD_CLOSE_TO_LAST_HOUSE = 2.5;
const double REWARD_TC_INFLUENCE_FACTOR = 3.0;
const double PENALTY_TOO_MANY_ROADSIDES = -10.0;
...
std::vector<std::vector<bool>> bfs_road_reachable(const std::vector<std::vector<int>>& grid, const std::vector<Pos>& start_building_coords);
bool is_connected_by_road(const std::vector<std::vector<int>>& grid, const std::vector<Pos>& start_building_coords, const std::vector<Pos>& target_building_coords);
...
public:
    ...
    int _fill_gaps_greedy() {
        ...
        for (int _ = 0; _ < max_attempts; ++_) {
            ...
            if (_check_all_buildings_connectivity(temp_grid, best_gap.r, best_gap.c)) {
    ...
    State reset() {
        ...
        for (const auto& pos : possible_tc_placements) {
            if (!check_overlap(pos.r, pos.c, tc_h, tc_w, placed_buildings_info_) &&
                ...
                if (is_connected_by_road(temp_grid, kontor_coords_list_, potential_tc_coords)) {
    ...
    bool _check_all_buildings_connectivity(std::vector<std::vector<int>>& current_grid, int new_house_r = -1, int new_house_c = -1) {
        std::vector<PlacedBuilding> temp_placed_buildings_info = placed_buildings_info_;
        if (new_house_r != -1) {
            int house_h = HOUSE_H;
            int house_w = HOUSE_W;
            place_building_on_grid(current_grid, HOUSE, new_house_r, new_house_c, house_h, house_w);
            temp_placed_buildings_info.push_back({new_house_r, new_house_c, house_h, house_w, HOUSE});
        }

        
        if (!is_connected_by_road(current_grid, kontor_coords_list_, tc_coords_list_)) {
            return false;
        }

        std::vector<std::vector<bool>> current_kontor_reach = bfs_road_reachable(current_grid, kontor_coords_list_);
        std::vector<std::vector<bool>> current_tc_reach = bfs_road_reachable(current_grid, tc_coords_list_);

        for (const auto& building : temp_placed_buildings_info) {
            if (building.type == HOUSE) {
                std::vector<Pos> house_coords = get_building_coords(building.r, building.c, building.h, building.w);
                
                bool connected_to_kontor = false;
                for (const auto& h_coord : house_coords) {
                    for (const auto& dir : {Pos{-1,0}, Pos{1,0}, Pos{0,-1}, Pos{0,1}}) {
                        int nr = h_coord.r + dir.r;
                        int nc = h_coord.c + dir.c;
                        if (nr >= 0 && nr < rows_ && nc >= 0 && nc < cols_ && current_grid[nr][nc] == ROAD && current_kontor_reach[nr][nc]) {
                ...
                for (const auto& h_coord : house_coords) {
                    for (const auto& dir : {Pos{-1,0}, Pos{1,0}, Pos{0,-1}, Pos{0,1}}) {
                        int nr = h_coord.r + dir.r;
                        int nc = h_coord.c + dir.c;
                        if (nr >= 0 && nr < rows_ && nc >= 0 && nc < cols_ && current_grid[nr][nc] == ROAD && current_tc_reach[nr][nc]) {
    ...
    double _calculate_reward_shaping(int r, int c) {
        ...
        if (adjacent_roads > 2) {
            reward_bonus += PENALTY_TOO_MANY_ROADSIDES;
        }
    ...
    std::tuple<State, double, bool> step(int action_idx) {
        ...
        std::vector<std::vector<int>> temp_grid = grid_;
        if (_check_all_buildings_connectivity(temp_grid, r, c)) {
...
std::vector<std::vector<bool>> bfs_road_reachable(const std::vector<std::vector<int>>& grid, const std::vector<Pos>& start_building_coords) {
    ...
    for (const auto& br_bc : start_building_coords) {
        for (const auto& dir : {Pos{-1,0}, Pos{1,0}, Pos{0,-1}, Pos{0,1}}) {
            ...
            if (nr >= 0 && nr < rows && nc >= 0 && nc < cols && grid[nr][nc] == ROAD) {
...
bool is_connected_by_road(const std::vector<std::vector<int>>& grid, const std::vector<Pos>& start_building_coords, const std::vector<Pos>& target_building_coords) {
    std::vector<std::vector<bool>> reachable_roads = bfs_road_reachable(grid, start_building_coords);
    ...
    for (const auto& tr_tc : target_building_coords) {
        for (const auto& dir : {Pos{-1,0}, Pos{1,0}, Pos{0,-1}, Pos{0,1}}) {
            ...
            if (nr >= 0 && nr < rows && nc >= 0 && nc < cols && grid[nr][nc] == ROAD && reachable_roads[nr][nc]) { 
                return true;
            }
        }
    }
...
int main() {
    ...
    EnhancedCityPlanningEnv env(GRID_WIDTH, GRID_HEIGHT);
    ...
    std::cout << "Enhanced QRL-Training on a " << GRID_WIDTH << "x" << GRID_HEIGHT << " grid..." << std::endl;
